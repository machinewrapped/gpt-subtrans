...
def GetThemeNames():
    themes : list[str] = []
    theme_path : str = GetResourcePath("theme")
...
def LoadStylesheet(name : str|None) -> str:
    if not name or name == "default":
        name = "subtrans-dark" if darkdetect.isDark() else "subtrans"

    filepath : str = GetResourcePath("theme", f"{name}.qss")
...
def ClearForm(layout : QFormLayout):
    ...
    while layout.rowCount():
        row : QFormLayout.TakeRowResult = layout.takeRow(0)
        if row.fieldItem:
            widget : QWidget = row.fieldItem.widget()
            if widget:
                widget.deleteLater()
        if row.labelItem:
            widget : QWidget = row.labelItem.widget()
            if widget:
                widget.deleteLater()